...
class MEDSPytorchDataset(torch.utils.data.Dataset):
    ...
    def load_subject_data(
        ...
        Examples:
            ...
            Test case with null static data - this should work without throwing AttributeError:
            >>> # Test null handling directly by using the fixed logic
            >>> import polars as pl
            >>> # Simulate null static data scenario
            >>> test_df = pl.DataFrame({
            ...     "static_code": [None],
            ...     "static_numeric_value": [None]
            ... })
            >>> # Test our null-safe logic
            >>> static_code_item = test_df["static_code"].item()
            >>> static_code = static_code_item.to_list() if static_code_item is not None else []
            >>> static_numeric_value_item = test_df["static_numeric_value"].item()
            >>> static_numeric_value = static_numeric_value_item.to_list() if static_numeric_value_item is not None else []
            >>> static_code  # Should be empty list instead of failing
            []
            >>> static_numeric_value  # Should be empty list instead of failing
            []
        ...
        # Handle null static data gracefully
        static_code_item = subj_schema["static_code"].item()
        static_code = static_code_item.to_list() if static_code_item is not None else []
        
        static_numeric_value_item = subj_schema["static_numeric_value"].item()
        static_numeric_value = static_numeric_value_item.to_list() if static_numeric_value_item is not None else []
        ...
        match self.config.static_inclusion_mode:
            ...
            case StaticInclusionMode.INCLUDE:
                try:
                    ...
                    static_data = {
                        'dim1/mask': np.empty((len(batch),0), dtype=np.bool_),
                        'static_code': np.empty((len(batch),0), dtype=np.uint8),
                        'static_numeric_value': np.empty((len(batch),0), dtype=np.float32)
                        }
